...
%>
<h3>JSP의 EL을 이용하여 파라미터의 속성값 출력</h3>
...
<%-- ${test} : 영역담당 객체에 저장된 속성 중 이름이 test인 속성의 값을 리턴
		...
		1.pageContext 객체에 test 속성검색, 존재하면 속성값 확인
		2.1번에 속성이 없으면
			request 객체에 test 속성검색.존재하면 속성값 리턴
		3.2번에 속성이 없으면
			session 객체에 test 속성검색,존재함 속성값 리턴
		4.3번에 속성이 없으면
			application 객체에 test 속성검색,존재함 속성값 리턴
		5.4번에 속성이 없으면
			아무것도 출력하지 않는다. null도 출력하지 않음. 오류발생도 없음
		직접 영역담당 객체를 지정할 수 있다.
		pageContext 객체의 속성 : ${pageContext.test}
